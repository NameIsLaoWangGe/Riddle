(function () {
    ...
    (function (lwg) {
        ...
        (function (Admin) {
            ...
            function _openCustomsScene() {
                ...
                _openScene(sceneName, null, null, null);
            }
            ...
            function _refreshScene() {
                ...
                Admin._sceneControl[sceneName].close();
                _openScene(sceneName, null, null, null);
            }
            Admin._refreshScene = _refreshScene;
            function _closeScene(gameLevel) {
                let sceneName;
                if (gameLevel <= 9) {
    ...
    class UIMain_StaticDog extends lwg.Admin.Person {
        lwgInit() {
        }
        onDisable() {
        }
    ...
    class UIMain_Gongzhu extends lwg.Admin.Person {
        ...
        dogAndPerson(other, self) {
            ...
            if (this.buffState === lwg.Enum.BuffState.stick) {
                ...
                if (otherOwnerName.substring(0, 1) === 's') {
                    otherOwner['UIMain_Follow'].skeleton.play(lwg.Enum.dogAni.walk, true);
                }
        ...
        attackComplete(otherOwner) {
            let otherOwnerName = otherOwner.name;
            if (otherOwnerName.substring(0, 1) === 's') {
                let dogSk = otherOwner['UIMain_Follow'].skeleton;
    ...
    class Dog extends UIMain_Gongzhu {
        ...
        move() {
            if (this.eatFood) {
                ...
                if (this.moveDirection === lwg.Enum.PersonDir.left) {
                    this.rig.setVelocity({ x: -2.1, y: 0 });
                    this.skeleton.scaleX = 1;
                }
                else if (this.moveDirection === lwg.Enum.PersonDir.right) {
                    this.rig.setVelocity({ x: 2.1, y: 0 });
    ...
    class UIMain_Person extends Laya.Script {
        constructor() { super(); }
        onEnable() {
        }
        onDisable() {
    ...
    class UIMain_Follow extends lwg.Admin.Object {
        ...
        lwgInit() {
            this.self = this.owner;
            let parent = this.self.parent;
            if (this.self.name.substring(0, 9) === 'staticDog') {
                this.firstPos.y = parent.height - this.self.height / 2 - 9;
                this.createskeleton_StaticDog();
                if (this.self.x > parent.width / 2) {
                    this.skeleton.scaleX = 1;
                }
                else {
                    this.skeleton.scaleX = -1;
    ...
    class UIVictory extends lwg.Admin.Scene {
        ...
        btnLastUp() {
            ...
            lwg.Admin._openScene(sceneName, 1, this.self, f => { });
    ...
    class GameConfig {
        ...
        static init() {
            ...
            reg("script/Game/UIMain_Person.ts", UIMain_Person);
